...
/**
 * Creates realistic sample forensic evidence files for immediate in-browser testing
 * including Windows Security Event Logs (.evtx), Outlook Mailbox (.pst), and Recent Shortcuts (.lnk).
 */
export function createSampleForensicFiles(): File[] {
  // 1. Windows Security Event Log (.evtx)
...
`;
  ...
  // Create EVTX header (ElfFile\0) + UTF-8 payload
  ...
  // 2. Outlook Mailbox Archive (.pst)
  const pstHeader = Buffer.alloc(512);
  // Magic: !BDN (0x21 0x42 0x44 0x4E)
  pstHeader[0] = 0x21;
  pstHeader[1] = 0x42;
  pstHeader[2] = 0x44;
  pstHeader[3] = 0x4e;
  pstHeader.writeUInt16LE(23, 10); // Unicode PST version
  pstHeader[461] = 0; // Unencrypted stream payload for test predictability
...
`;
  ...
  // 3. Recent Files & Shortcuts (.lnk)
  const lnkBuffer = Buffer.alloc(2048);
  // Header magic for LNK: 0x4C 0x00 0x00 0x00 (size of Shell Link Header = 76 bytes)
  lnkBuffer[0] = 0x4c;
  lnkBuffer[1] = 0x00;
  lnkBuffer[2] = 0x00;
  lnkBuffer[3] = 0x00;
  // Write target paths in both UTF-16LE and ASCII
  ...
  lnkBuffer.write(targetPath4, 1000, 'ascii');
